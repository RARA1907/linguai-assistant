...
export default function Sidebar({ conversations, activeId, onSelect, onNew, onDelete, onTopicClick }: SidebarProps) {
  return (
    <aside
      style={{
        width: '260px',
      ...
      <div style={{ padding: '12px 16px', borderTop: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
          {TOPICS.map((topic) => (
            <button
              key={topic}
              onClick={() => onTopicClick?.(`Give me an academic overview of ${topic} in linguistics`)}
              style={{
                fontSize: '10px',
                padding: '3px 7px',
